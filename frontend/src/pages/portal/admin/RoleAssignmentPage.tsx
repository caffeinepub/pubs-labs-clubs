...
export default function RoleAssignmentPage() {
  ...
  // TEMP: admin-only guard disabled for testing — skipping `if (!isAdmin)` check
  // Original guard:
  // if (!isAdmin) {
  //   return (
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertDescription>
          When someone signs in with Internet Identity and accepts the permission request, they are only{' '}
          <strong>authenticated</strong>. This does not automatically grant them Admin privileges. Admin
          access must be explicitly assigned by an existing administrator using this page.
        </AlertDescription>
      </Alert>
                      ...
                      <TableRow key={principalString}>
                        ...
                        <TableCell>
                          <Select
                            value={user.role}
                            onValueChange={(value) =>
                              handleRoleChange(principalString, value as UserRole)
                            }
                            disabled={isChanging}
                          >
