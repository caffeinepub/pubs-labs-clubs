...
import { Clock, PlusCircle, RefreshCw, Link2, AlertCircle, History } from 'lucide-react';
...
function operationLabel(op: Variant_link_create_update): { label: string; icon: React.ReactNode; variant: 'default' | 'secondary' | 'outline' } {
  switch (op) {
    case Variant_link_create_update.create:
      return { label: 'Created', icon: <PlusCircle className="h-3.5 w-3.5" />, variant: 'default' };
    case Variant_link_create_update.update:
      return { label: 'Updated', icon: <RefreshCw className="h-3.5 w-3.5" />, variant: 'secondary' };
    case Variant_link_create_update.link:
      return { label: 'Linked', icon: <Link2 className="h-3.5 w-3.5" />, variant: 'outline' };
    default:
      return { label: 'Changed', icon: <RefreshCw className="h-3.5 w-3.5" />, variant: 'outline' };
  }
}

export default function ChangeHistoryPanel({ recordId }: ChangeHistoryPanelProps) {
      ...
      <CardContent>
        ...
        {!isLoading && !error && (!events || events.length === 0) && (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-xs text-muted-foreground/60 mt-1">Changes to this record will appear here.</p>
          </div>
        )}
        ...
        {!isLoading && !error && events && events.length > 0 && (
          <ol className="relative border-l border-border ml-3 space-y-0">
            {[...events].reverse().map((event, idx) => {
              ...
              return (
                <li key={`${String(event.id)}-${idx}`} className="mb-6 ml-4 last:mb-0">
                  {/* Timeline dot */}
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <Badge variant={op.variant} className="gap-1 text-xs py-0.5">
                      {op.icon}
                      {op.label}
                    </Badge>
                  ...
                  {event.changedFields && event.changedFields.length > 0 && (
                    <p className="text-xs text-muted-foreground mb-1">
                      Fields: <span className="font-medium text-foreground">{event.changedFields.join(', ')}</span>
                    </p>
                  )}
